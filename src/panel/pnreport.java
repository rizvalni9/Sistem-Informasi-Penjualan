...
public class pnreport extends javax.swing.JPanel implements viewUI{
    ...
    public void engineV(){
        if(getSlcV()==1){
            ...
            mnl_content.repaint();
            mnl_content.revalidate();
            cbxM.addItem("Select Item...");
            try{
                rs=con.adapter.executeQuery("SELECT DISTINCT SUBSTRING(tgltrs, 4, 10) AS tgltrs FROM nota ORDER BY tgltrs");
                while(rs.next()){
                    cbxM.addItem(rs.getString("tgltrs"));
                }
            }catch(Exception e){
            ...
            mnl_content.repaint();
            mnl_content.revalidate();
            cbxY.addItem("Select Item...");
            try{
                rs=con.adapter.executeQuery("SELECT DISTINCT SUBSTRING(tgltrs, 7, 10) AS tgltrs FROM nota ORDER BY tgltrs");
                while(rs.next()){
                    cbxY.addItem(rs.getString("tgltrs"));
                }
            }catch(Exception e){
